    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                name1=(String) stock1.get("name");
                name2=(String) stock2.get("name");
                date1=(Date) stock1.get("date");
                date2=(Date) stock2.get("date");
                if(stock1.get("change")!=null)   change1=(double) stock1.get("change");
                else{
                    double last=0.0;
                    double open=0.0;
                    if (stock1.get("last")!=null)  last=(double) stock1.get("last");
                    if (stock1.get("open")!=null)  open=(double) stock1.get("open");
                    change1=last-open;
                }

                if(stock2.get("change")!=null)  change2=(double) stock2.get("change");
                else{
                    double last=0.0;
                    double open=0.0;
                    if (stock2.get("last")!=null)  last=(double) stock2.get("last");
                    if (stock2.get("open")!=null)  open=(double) stock2.get("open");
                    change2=last-open;
                }

                if(name1.compareTo(name2)!=0) return name1.compareTo(name2);
                else if(date2.compareTo(date1)!=0) return date2.compareTo(date1);
                else return (change2.compareTo(change1));
