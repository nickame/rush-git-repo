...
public class Arcanoid
{
    ...
    {
        drawBoders(canvas);
        this.game.getStand().draw(canvas);
        for(int i=0; i<this.getBricks().size(); i++){
            this.getBricks().get(i).draw(canvas);
        }
    ...
    public void checkBricksBump()
    {
       ...
       if(game.getBall().isIntersec(game.getBricks().get(i))){
           double angel = Math.random() * 360;
           ball.setDirection(angel);
           game.getBricks().remove(i);
       }
    ...
    {
        if(game.getBall().getY()>game.getHeight()) isGameOver=true;
